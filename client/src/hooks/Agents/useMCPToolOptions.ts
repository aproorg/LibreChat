...
/**
 * Persists `set` for one flag relative to the effective default, so an entry is
 * only written when it diverges from that default and is cleared when it matches
 * (letting a later server-side default change take effect). A `serverDefault` of
 * `true` inverts which of the two states a missing key can express, which is
 * exactly {@link withNativeBooleanOptOut}.
 */
function applyBooleanOption(
  ...
  return serverDefault
    ? withNativeBooleanOptOut(options, toolId, key, set)
    : withBooleanOption(options, toolId, key, set);
}
...
function useBooleanToolOption(
  ...
  const isSet = useCallback(
    (toolId: string, serverDefault = false): boolean =>
      (formToolOptions?.[toolId]?.[key] ?? serverDefault) === true,
    [formToolOptions, key],
  );
  ...
  const toggle = useCallback(
    (toolId: string, serverDefault = false) => {
      ...
      const set = (currentOptions[toolId]?.[key] ?? serverDefault) !== true;
      setValue(
        'tool_options',
        applyBooleanOption(currentOptions, toolId, key, set, serverDefault),
        {
          shouldDirty: true,
        },
      );
  ...
  const toggleAll = useCallback(
    (tools: AgentToolType[], serverDefault = false) => {
      ...
      for (const tool of tools) {
        updatedOptions = applyBooleanOption(updatedOptions, tool.tool_id, key, set, serverDefault);
      }
