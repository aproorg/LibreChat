...
const getOptions = async ({ req, endpointOption }) => {
  ...
  let credentials = isUserProvided
    ? await getUserKey({ userId: req.user.id, name: EModelEndpoint.bedrockAgents })
    : {
  ...
  if (expiresAt && isUserProvided) {
    checkUserKeyExpiry(expiresAt, EModelEndpoint.bedrockAgents);
  }
  ...
  const bedrockConfig = req.app.locals[EModelEndpoint.bedrockAgents];
